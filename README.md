# Sandwichboy

A game representing my life of the past 3 or so years. I will not be expanding on this statement.


Navigate to https://icodevery1337.github.io/Sandwichboy/ to play it online.

## Running locally
Make sure you have python installed.

```console
git clone git@github.com:ICodeVery1337/Sandwichboy.git
cd Sandwichboy
python -m http.server 8888
```

And then navigate to http://localhost:8888/ 

## Music

I do not own any of the music or sounds - purely used for entertainment purposes. Please support the original creators.

The music used is:

- Tateyuki Shigaraki - Joking Motive
- Bag Raiders - Shooting Stars

Memes used:
- My Longest Yeah Boy Ever
- Fahh
- Anime 'Wow' Sound Effect
- You Just Have to Say That You're Fine
- Michael Rosen - Nice

# Prettier
Used to format my terrible Javascript code:

## Install
```console
npm install --save-dev --save-exact prettier
```

## Use
```console
npx prettier . --write
```
